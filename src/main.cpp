...
using namespace std; // Added for simplicity of coming code
    ...
    if (sdata.size() > 2 && sdata[0] == '4' && sdata[1] == '2') {
        ...
        if (event == "telemetry") {
          ...
          */
		  ...
		  double * ptrx = &ptsx[0];
		  Eigen::Map<Eigen::VectorXd> ptsxv(ptrx, 6);
		  double * ptry = &ptsy[0];
		  ...
		  /*for (unsigned i = 0; i < ptsx.size(); ++i) {
			  next_x_vals.push_back(ptsxv(i));
			  next_y_vals.push_back(ptsyv(i));
		  }*/
		  double poly_inc = 2.5;
		  int num_points = 25;
